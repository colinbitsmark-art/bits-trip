...
refs.swapBtn.addEventListener("click", () => {
  ...
  saveCurrentForm();
  enderResults();(()=>{const b=document.querySelector(".account-btn");if(!b||document.querySelector("#authDialog"))return;const z=document.createElement("style");z.textContent=".account-btn.signed-in{color:#fff;background:var(--brand-strong)}.auth-dialog{width:min(560px,calc(100% - 28px))}.auth-icon{background:var(--brand)}.auth-tabs{display:grid;grid-template-columns:1fr 1fr;gap:6px;margin:22px 0;padding:4px;background:#edf5fb;border-radius:8px}.auth-tabs button{min-height:42px;color:#405166;background:transparent;border:0;border-radius:8px;font-weight:900}.auth-tabs button.active{color:#fff;background:var(--brand)}.auth-fields{display:grid;gap:14px}.auth-field{display:grid;gap:7px;color:#44566b;font-size:14px;font-weight:800}.auth-field[hidden]{display:none}.auth-field input{min-height:46px;padding:0 13px;color:var(--ink);background:#f8fbfd;border:1px solid #c9d8e5;border-radius:8px;outline:none}.auth-field input:focus{border-color:var(--brand);box-shadow:0 0 0 3px rgba(0,120,212,.12)}.auth-check{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:14px;font-weight:700}.auth-check input{width:16px;height:16px;accent-color:var(--brand)}.auth-message{min-height:22px;margin:14px 0 18px;color:#c2410c;font-size:14px;font-weight:800}";document.head.append(z);const d=document.createElement("dialog");d.className="booking-dialog auth-dialog";d.id="authDialog";d.innerHTML='<form class="auth-form" novalidate><div class="dialog-head"><span class="dialog-icon auth-icon">B</span><div><h2 id="authTitle">\u767b\u5f55 bits trip</h2><p id="authSubtitle">\u4f7f\u7528\u90ae\u7bb1\u7ee7\u7eed\u7ba1\u7406\u8ba2\u5355\u548c\u865a\u62df\u652f\u4ed8\u8bb0\u5f55\u3002</p></div></div><div class="auth-tabs" role="tablist" aria-label="\u8d26\u53f7\u64cd\u4f5c"><button class="active" type="button" role="tab" aria-selected="true" data-auth-mode="login">\u767b\u5f55</button><button type="button" role="tab" aria-selected="false" data-auth-mode="register">\u6ce8\u518c</button></div><div class="auth-fields"><label class="auth-field" data-auth-register hidden><span>\u59d3\u540d</span><input id="authName" autocomplete="name"></label><label class="auth-field"><span>\u90ae\u7bb1</span><input id="authEmail" type="email" autocomplete="email" required></label><label class="auth-field"><span>\u5bc6\u7801</span><input id="authPassword" type="password" autocomplete="current-password" minlength="6" required></label><label class="auth-check"><input id="authRemember" type="checkbox" checked><span>\u8bb0\u4f4f\u767b\u5f55\u72b6\u6001</span></label></div><p class="auth-message" id="authMessage" aria-live="polite"></p><menu><button value="close" class="quiet-btn" type="button" data-auth-close>\u53d6\u6d88</button><button class="search-btn" type="submit" id="authSubmit">\u767b\u5f55</button></menu></form>';document.body.append(d);const q=s=>d.querySelector(s),a=s=>[...d.querySelectorAll(s)],r={f:q(".auth-form"),t:q("#authTitle"),s:q("#authSubtitle"),tabs:a("[data-auth-mode]"),nf:q("[data-auth-register]"),n:q("#authName"),e:q("#authEmail"),p:q("#authPassword"),m:q("#authMessage"),go:q("#authSubmit"),x:q("[data-auth-close]")};let mode="login";function clear(){if(location.hash==="#account")history.replaceState(null,"",location.pathname+location.search)}function set(x){mode=x;const reg=x==="register";r.t.textContent=reg?"\u6ce8\u518c bits trip":"\u767b\u5f55 bits trip";r.s.textContent=reg?"\u521b\u5efa\u8d26\u53f7\u540e\u53ef\u4fdd\u5b58\u884c\u7a0b\u548c\u865a\u62df\u652f\u4ed8\u8bb0\u5f55\u3002":"\u4f7f\u7528\u90ae\u7bb1\u7ee7\u7eed\u7ba1\u7406\u8ba2\u5355\u548c\u865a\u62df\u652f\u4ed8\u8bb0\u5f55\u3002";r.nf.hidden=!reg;r.n.required=reg;r.p.autocomplete=reg?"new-password":"current-password";r.go.textContent=reg?"\u521b\u5efa\u8d26\u53f7":"\u767b\u5f55";r.m.textContent="";r.tabs.forEach(t=>{const on=t.dataset.authMode===x;t.classList.toggle("active",on);t.setAttribute("aria-selected",String(on))})}function open(x=mode){set(x);if(location.hash!=="#account")history.replaceState(null,"",location.pathname+location.search+"#account");typeof d.showModal==="function"?d.showModal():d.setAttribute("open","");r.e.focus()}function close(){if(d.open)d.close();clear()}r.tabs.forEach(t=>t.addEventListener("click",()=>set(t.dataset.authMode)));r.x.addEventListener("click",close);d.addEventListener("cancel",clear);d.addEventListener("close",clear);r.f.addEventListener("submit",e=>{e.preventDefault();const mail=r.e.value.trim(),pass=r.p.value.trim(),name=r.n.value.trim()||mail.split("@")[0]||"\u8d26\u6237";if(!mail||!mail.includes("@")){r.m.textContent="\u8bf7\u8f93\u5165\u6709\u6548\u90ae\u7bb1\u3002";r.e.focus();return}if(pass.length<6){r.m.textContent="\u5bc6\u7801\u81f3\u5c11 6 \u4f4d\u3002";r.p.focus();return}b.textContent="\u8d26\u6237 \u00b7 "+name;b.classList.add("signed-in");close()});b.addEventListener("click",()=>open(mode));window.addEventListener("hashchange",()=>{if(location.hash==="#account"&&!d.open)open(mode)});if(location.hash==="#account")open(mode)})();
